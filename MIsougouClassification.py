...
public static int labelToInt(String label){
		if(label.equals("C000007")){
			return 1;
		}else if(label.equals("C000008")){
			return 2;
		}else if (label.equals("C000010")) {
			return 3;
		}else if (label.equals("C000013")) {
			return 4;
		}else if (label.equals("C000014")) {
			return 5;
		}else if (label.equals("C000016")) {
			return 6;
		}else {
			return -1;
		}
...
if __name__ == '__main__':
    filePath = 'sougouNewsMI.txt'
    wfilePath = 'resultOfLDAsougou_miSVM.txt'
    ...
    allLabelR = []
    allLabelF = []
    for i in range(1,6): # for each label
        usedData = []
        positiveExamples = []
        negativeExamples = []
        print "label:" + str(i)
        ...
        for fold in range(0,10): # 10 fold validate
            ...
            for k in range(0,len(usedData)):
                if k >= fold*chunkSize and k < (fold + 1) * chunkSize:
                    testData = usedData[k][0]
                    testLabel = usedData[k][1]
                else:
                    trainData = usedData[k][0]
                    trainLabel = usedData[k][1]
